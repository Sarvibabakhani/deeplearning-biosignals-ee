...
from ResNetAtt_modules import ResNet1D_Att, root_mean_squared_error
from Train_evaluate_ResNetAtt import train_model, evaluate_model,evaluate_model_by_activity
...
def run_resnet_attention(sig,epochs=100):
            rmse_list = []
            ...
            for i, test_df in enumerate(df_list2):
                    ...
                    num_time_steps = 10  # Number of time steps for sequence
                    ...
                    model = model = ResNet1D_Att(input_channels=len(sig), output_size=num_time_steps).to(device)
                    
                    # Training Setup
                    criterion = root_mean_squared_error
                    optimizer = optim.Adam(model.parameters(), lr=0.0005,betas=(0.88,0.999))
...
def run_resnet_attention_perActivity(sig,epochs=100):
        ...
        for i, test_df in enumerate(df_list2):
                ...
                device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                model = ResNet1D_Att(input_channels=len(sig), output_size=num_time_steps).to(device)
