...
def run_transformer(sig,epochs=100):
            rmse_list = []
            ...
            for i, test_df in enumerate(df_list2):
                    ...
                    model = MultiheadAttention1DModel(input_channels=len(sig), output_size=num_time_steps).to(device)

                    # Training Setup
                    criterion = root_mean_squared_error
                    optimizer = optim.Adam(model.parameters(), lr=0.0009,betas=(0.9,0.999))
...
def run_transformer_perActivity(sig,epochs=100):
