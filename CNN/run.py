...
from Train_evaluate_CNN import train_model, evaluate_model,evaluate_model_by_activity
...
def run_CNN(sig,epochs=180):
            ...
            for i, test_df in enumerate(df_list2):
                    ...
                    model = CNNModel(input_channels=len(sig), num_time_steps=num_time_steps ,output_size=num_time_steps).to(device)
...
def run_CNN_perActivity(sig,epochs=180):
        ...
        for i, test_df in enumerate(df_list2):
                    ...
                    # Model, Loss, Optimizer
                    model = CNNModel(input_channels=len(sig), output_size=num_time_steps).to(device)
                    criterion = root_mean_squared_error
                    optimizer = optim.Adam(model.parameters(), lr=0.0005, betas=(0.9, 0.995))
                    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.4, patience=15)
                    early_stopping_patience = 40
